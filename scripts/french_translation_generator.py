...
def build_prompt(word: str, cat1: str, target_lang: str = "French") -> str:
...
# ───────────────────────────────
# Main French Translation Pipeline
# ───────────────────────────────

async def run_french_translation_pipeline(conn, batch_size=500):
    """
    Translates English words to French using adaptive GPT model selection
    """
    cur = conn.cursor()

    # STEP 1 — fetch entries needing French translation
    cur.execute("""
    ...
    if total == 0:
        ...
        print("[FRENCH] No entries found. Nothing to process.", flush=True)
    ...
    print(f"[FRENCH] Starting job {job_id}. Total entries: {total}", flush=True)
    ...
    for idx, (hash_id, word, cat1, freq, level) in enumerate(rows, start=1):
        ...
        if model == "SKIP":
            ...
            if key in translation_cache:
                ...
                prompt = build_prompt(word, cat1, "French")
                translation = get_translation(prompt, model)
                translation_cache[key] = translation

        # Insert into translations_french table
        cur.execute("""
            INSERT INTO translations_french (hash_id, translation, word_id)
            VALUES (%s, %s, %s)
        ...
        if idx % 25 == 0:
            ...
            print(f"[FRENCH] {idx}/{total} processed...", flush=True)
    ...
    print(f"[FRENCH] Job {job_id} DONE.", flush=True)
...
async def run_french_translation_pipeline_streaming(conn, batch_size=500):
    """
    ...
    if total == 0:
        ...
        yield "[FRENCH] No entries found. nothing_to_process"
        return
    ...
    yield f"[FRENCH] Starting job {job_id}. Total entries: {total}"
    ...
    for idx, (hash_id, word, cat1, freq, level) in enumerate(rows, start=1):
        ...
        if idx % 25 == 0:
            ...
            yield f"[FRENCH] {idx}/{total} processed..."
    ...
    yield f"[FRENCH] Job {job_id} DONE."
