...
async def run_german_translation_pipeline(conn, batch_size=500):
    ...
    cur.execute("""
        ...
        LEFT JOIN translations_german tg ON cl.hash_id = tg.hash_id
        WHERE tg.translation IS NULL OR tg.translation = ''
        LIMIT %s
    """, (batch_size,))
    ...
    for idx, (hash_id, word, cat1, freq, level) in enumerate(rows, start=1):
        ...
        # Insert into translations_german table
        cur.execute("""
            INSERT INTO translations_german (hash_id, translation, word_id)
            VALUES (%s, %s, %s)
